...
class DMAHandler(w: Int, p: PlatformWrapperParams) extends Module {
  ...
  for ( i <- 0 until numCompareUnits ) yield {
  ...
  // printf("state %d      threshcount %d     elemcount  %d     index %d\n", rState, io.threshCount, io.elemCount, rIndex);
  ...
  switch (rState) {
    ...
    is (sReaderFlush) {
      when (rBytesLeft === UInt(bytesPerElem)) {
        rState := sFinished
      }
      .otherwise {
        when (io.reader.out.valid) {
