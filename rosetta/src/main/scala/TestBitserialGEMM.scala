...
class TestBitserialGEMM() extends RosettaAccelerator {
  ...
  val lhs_reader = make_reader(
    port = 0,
    baseAddr = io.lhs_addr + row_lhs * bytes_per_row + lbit * lhs_bytes_per_bitplane,
    byteCount = bytes_per_row,
    start = Bool(false))

  val rhs_reader = make_reader(
    port = 1,
  ...
  val s_idle :: s_inner :: s_inner_test :: s_inner_post :: s_rbit_test :: s_lbit_test :: s_row_lhs_test :: s_row_rhs_test :: s_wait :: s_done :: Nil =
    Enum(UInt(), 10)
