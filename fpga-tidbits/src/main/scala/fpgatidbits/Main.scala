...
object MainObj {
  ...
  val accelMap: AccelMap  = Map(
    ...
    // QBART entry-level accelerator units.
    "TestDMAThresholder" -> {p => new TestDMAThresholder(p)},
    "TestDMAWriter" -> {p => new TestDMAWriter(p)},
    "TestThresholder" -> {p => new TestThresholder(p)}
  )
  ...
  def makeEmulator(args: Array[String]) = {
    ...
    val testRoot = "src/main/cpp/platform-wrapper-tests-qbart/"
